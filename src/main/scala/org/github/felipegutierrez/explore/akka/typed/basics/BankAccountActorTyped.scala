...
object BankAccountActorTyped {
  ...
  def run(): Unit = {
    ...
    val bankAccount: Behavior[BankAccountTxn] = Behaviors.setup { context =>
      var totalBalance = 0
    ...
    val bankAccountActor = ActorSystem(bankAccount, "BankAccountSystem")
