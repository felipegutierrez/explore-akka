...
object Dispatchers extends App {
  ...
  def run() = {
    ...
    val dbActorPoolMaster = system.actorOf(RoundRobinPool(5).props(Props[DBActorSolution2]), "dbActorPoolMaster")
    for (i <- 1 to 1000) {
      dbActorPoolMaster ! s"message [$i] to update database!"
    }
  }
  ...
  class DBActorSolution2 extends Actor with ActorLogging {
    override def receive: Receive = {
      case message =>
        Thread.sleep(5000)
        log.info(s"success: $message")
