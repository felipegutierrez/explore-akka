...
object StreamCustomGraphShapes {
  ...
  def run() = {
     ...
     */
    def balanceMxNImpl[T](inputPorts: Int, outputPorts: Int) = {
      GraphDSL.create() { implicit builder =>
        import GraphDSL.Implicits._
        val mergeShape = builder.add(Merge[T](inputPorts))
        val balanceShape = builder.add(Balance[T](outputPorts))
    ...
    val balanceMxNGraph = RunnableGraph.fromGraph(
      GraphDSL.create() { implicit builder =>
        ...
        def createSink(index: Int) = Sink.fold(0)((count: Int, element: Int) => {
          println(s"generic balancer [sink $index] Received $element, current count is $count")
          count + 1
        })
        ...
        val balance2x3 = builder.add(balanceMxNImpl[Int](2, 3))
  ...
  case class BalanceMxN[T](override val inlets: List[Inlet[T]],
                           override val outlets: immutable.Seq[Outlet[T]]) extends Shape {
